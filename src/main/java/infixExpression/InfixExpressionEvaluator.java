...
public class InfixExpressionEvaluator {
    private static final Logger logger = (Logger) LoggerFactory.getLogger(InfixExpressionEvaluator.class);

    public static void main(String[] args) {
        String expression = "3 + 5 * (2 - 8)";
        try {
            double result = evaluate(expression);
            logger.info("The result of the expression is: {}" , result);
        } catch (Exception e) {
            logger.info("Error evaluating expression: {}" , e.getMessage());
        }
    }
